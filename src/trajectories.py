...
def collect_trajectories(env, nb, policy_x, policy_y):
    ...
    trajectories['info'] = []
    for _ in range(nb):
        states, actions_x, actions_y, rewards_x, rewards_y, info = collect_trajectory(env, policy_x, policy_y)
...
def collect_trajectory(env, policy_x, policy_y):
    ...
    while not done:
        ...
        infos.append(info)
    return torch.FloatTensor(states), torch.LongTensor(actions_x), torch.LongTensor(actions_y), torch.FloatTensor(rewards_x), torch.FloatTensor(rewards_y), infos
